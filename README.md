...
**M0 → M3 complete** (TDD) — pnpm monorepo; `@rasa/shared` Zod schemas (six spec §6 entities) + `SourceAdapter` moat boundary; `@rasa/server`'s Swiggy MCP stack (OAuth2.1/PKCE, 14 typed tool wrappers, deterministic `mock-swiggy-mcp`, `SwiggyAdapter.discover()`); the state store (`0001_init.sql` DDL + RLS, `RasaDb.withUser()`, repositories, pglite-backed integration tests); and the nutrition pipeline (portion table, INDB source, LLM canonicalizer behind `LlmClient`, hybrid enrich → estimated macros + confidence + cache). `pnpm test` (**99 tests**) green, `pnpm typecheck` + `pnpm lint` clean. Next: **M4** — Preferences Agent + onboarding prompt.
